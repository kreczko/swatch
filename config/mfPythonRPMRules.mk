...
# 
ifndef PythonModule
	$(error Python module name missing "PythonModule"
endif
...
_rpmbuild: _setup_update
	...
	# Change directory into lib and copy all share objects into the pkg
	cd lib && \
	find . -name "*.so" -exec install -m 644 \{\} ${RPMBUILD_DIR}/${PythonModule}/\{\} \;
	# Add a manifest file
	echo "include ${PythonModule}/*.so" > ${RPMBUILD_DIR}/MANIFEST.in
...
_setup_update:
	...
	sed -i 's#__module__#${PythonModule}#' ${RPMBUILD_DIR}/${PackageName}.py
